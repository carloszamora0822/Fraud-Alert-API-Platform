...
## Task 2.8 — Postman Collection

**What I built**: A complete Postman collection (v2.1 JSON) documenting all 10 API endpoints across 4 folders (Health, Auth, Accounts, Alerts) with auto-chaining variables and pre-response scripts.

**What I learned**:
- Postman collection format (v2.1 JSON schema) — how requests, folders, variables, and scripts are structured
- Collection-level auth inheritance — set Bearer token once, all requests use it unless overridden
- Post-response scripts — JavaScript that runs after a response to extract and store values (e.g., capturing JWT from login)
- Variable chaining — how `{{token}}`, `{{account_id}}`, `{{alert_uuid}}` flow between requests to create a testable sequence
- Disabled query params — a UX pattern for documenting optional filters without cluttering the default request

| Concept | Confidence |
|---------|-----------|
| Postman collection structure | Solid |
| Variable chaining across requests | Solid |
| Post-response scripts | Solid |
| API documentation as artifact | Solid |

**Highlights**:
- Recognized this as a "nice to have" and moved through it efficiently — shows good prioritization instincts
- **Sprint 2 complete** — 8 tasks shipping JWT auth, RBAC, pagination, filtering, rate limiting, error handling, 43 tests, and full API documentation
